...
这个项目提供了一个 **MinerU MCP 服务器** (`mineru-mcp`)，它基于 **FastMCP 3.x** 框架构建。其主要功能是作为 **MinerU API** 的接口，用于将文档转换为 Markdown格式。
...
核心依赖仅 3 个：
- `fastmcp>=3.0.0` — MCP 协议框架（内置 stdio/SSE/streamable-http transport）
- `python-dotenv>=1.0.0` — 环境变量管理
- `httpx>=0.24.0` — 统一的 async HTTP 客户端
...
> **注意**：推荐使用 `streamable-http` 传输模式（MCP 规范推荐），SSE 模式已被标记为 deprecated。
